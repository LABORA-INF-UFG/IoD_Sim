...
class IoDSimEnv(gym.Env):
    ...
    def reset(self):
        ...
        requests.post(f"{self.host}/simulation")
	...
	direction = self.action_mapping[action]
        ...
        data = {"drone_id": 0, "acceleration": {"x": direction[0], "y": direction[1], "z":direction[2]}, "time": 2}
        requests.post(f"{self.host}/change_position", json=data)
    ...
    def close(self):
        # Stop the simulation
        requests.delete(f"{self.host}/stop")
